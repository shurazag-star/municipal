class AgentAutonomousResolver
  ...
  def parent_activity_exists?(raw_code)
    ...
    candidates = @change_set.program_version.program_nodes.where(node_type: "activity")
    candidates = candidates.where(code: parsed[:activity_code]) if parsed[:activity_code].present?
    return true if candidates.any? { |node| parent_activity_matches?(node, parsed) }
    return true if parsed[:activity_code].present? && candidates.one?
